  ...
  <div>
    <md-card md-with-hover class="timeline" v-for="informacoes in info" v-bind="informacoes" :key="informacoes">
      <md-ripple>
        <md-card-header>
          <p>Tipo</p>
          <h2 class="md-title" > {{ informacoes.cardType }} </h2>
        </md-card-header>
        <md-card-content>
        </md-card-content>
      </md-ripple>
...
export default {
  ...
  mounted () {
    ...
    function stringToIcon () {
      let string = this.info.cardType
      if (string === 'EXPENSE') {
        return `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" width="80.13px" height="80.13px" viewBox="0 0 80.13 80.13" style="enable-background:new 0 0 80.13 80.13;" xml:space="preserve">
                  <g>
